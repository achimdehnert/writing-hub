...
class TestProjectListFilter:
    def setup_method(self):
        ...
        self.ct_roman, _ = ContentTypeLookup.objects.get_or_create(slug="roman", defaults={"name": "Roman", "order": 1})
        self.ct_story, _ = ContentTypeLookup.objects.get_or_create(
            slug="kurzgeschichte",
            defaults={"name": "Kurzgeschichte", "order": 2},
        )
        self.genre_fantasy, _ = GenreLookup.objects.get_or_create(name="Fantasy", defaults={"order": 1})
        self.genre_krimi, _ = GenreLookup.objects.get_or_create(name="Krimi", defaults={"order": 2})
        self.series = BookSeries.objects.create(title="Meine Serie", owner=self.user)
    ...
    def test_filter_by_genre(self):
        response = self.client.get(f"/projekte/?genre={self.genre_fantasy.pk}")
        assert response.status_code == 200
    ...
    def test_filter_by_content_type(self):
        response = self.client.get(f"/projekte/?typ={self.ct_story.pk}")
        assert response.status_code == 200
        titles = [p.title for p in response.context["projects"]]
    ...
    def test_filter_by_series(self):
        response = self.client.get(f"/projekte/?serie={self.series.pk}")
        assert response.status_code == 200
        titles = [p.title for p in response.context["projects"]]
        assert "Fantasy Roman" in titles
        assert len(titles) == 1
    ...
    def test_filter_combined_genre_and_type(self):
        response = self.client.get(f"/projekte/?genre={self.genre_fantasy.pk}&typ={self.ct_story.pk}")
        assert response.status_code == 200
        titles = [p.title for p in response.context["projects"]]
        assert "Fantasy Story" in titles
        assert "Fantasy Roman" not in titles
    ...
    def test_filter_only_shows_own_series(self):
        other = User.objects.create_user(username="otherfilteruser", password="pass123")
        BookSeries.objects.create(title="Fremde Serie", owner=other)
        response = self.client.get("/projekte/")
        series_titles = [s.title for s in response.context["series_options"]]
