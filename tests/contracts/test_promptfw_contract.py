...
class TestFrontmatterContract:
    ...
    def test_render_frontmatter_file_accepts_path_and_context(self):
        ...
        assert "file_path" in params, f"render_frontmatter_file must accept 'file_path', got {params}"
        assert "context" in params or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()), (
