...
NARRATIVE_MODELS = [
    {"code": "linear", "label": "Linear", "description": "A→B→C→D, Erzählzeit = Story-Zeit", "sort_order": 10},
    {
        "code": "in_medias_res",
        "label": "In medias res",
        "description": "Beginnt in der Mitte, dann Analepsen",
        "sort_order": 20,
    },
    {
        "code": "non_linear",
        "label": "Nicht-linear",
        "description": "Kapitel-Reihenfolge ≠ Story-Chronologie",
        "sort_order": 30,
    },
...
FORESHADOWING_TYPES = [
    ...
    {"code": "name", "label": "Name", "description": "Name der Figur oder des Ortes enthält Hinweis", "sort_order": 40},
    {
        "code": "verhalten",
        "label": "Verhalten",
        "description": "Charakterverhalten, das spätere Entscheidung vorausdeutet",
        "sort_order": 50,
    },
    {
        "code": "atmosphaere",
        "label": "Atmosphäre",
        "description": "Stimmung/Setting signalisiert kommendes Ereignis",
        "sort_order": 60,
    },
]
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for data in NARRATIVE_MODELS:
            obj, created = NarrativeModelLookup.objects.get_or_create(code=data["code"], defaults=data)
            if created:
        ...
        for data in FORESHADOWING_TYPES:
            obj, created = ForeshadowingTypeLookup.objects.get_or_create(code=data["code"], defaults=data)
            if created:
        ...
        for name, (c, u, s) in stats.items():
            self.stdout.write(self.style.SUCCESS(f"{name}: {c} erstellt, {u} aktualisiert, {s} übersprungen."))
