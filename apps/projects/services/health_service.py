...
def compute_dramaturgic_health(project) -> DramaturgicHealthResult:
    ...
    protagonists = list(
        project.character_links.filter(narrative_role="protagonist")
    )
    checks.append(HealthCheck(
    ...
    antagonists = list(
        project.character_links.filter(narrative_role="antagonist")
    )
    checks.append(HealthCheck(
    ...
    checks.append(HealthCheck(
        ...
        if hasattr(project, "subplot_arcs") else False,
